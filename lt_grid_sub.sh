...
py_script=$(readlink -f dumpleptau.py)
...
#lsetup "root 6.14.04-x86_64-slc6-gcc62-opt"
   ...
   if [ -s total_weights.root ] 
    then
      prun --exec "python dumpleptau.py -s branchList.txt %IN" --inDS $1 --outDS $out_name --outputs output.root --extFile total_weights.root --rootVer=6.18/04 --cmtConfig=x86_64-centos7-gcc8-opt
    else
      prun --exec "python dumpleptau.py -s branchList.txt %IN" --inDS $1 --outDS $out_name --outputs output.root --rootVer=6.18/04 --cmtConfig=x86_64-centos7-gcc8-opt
   fi
